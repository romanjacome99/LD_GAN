...
class M2S:
    def __init__(self, model, input_shape, stride, patch_size, mask_path=None, trainable_mask=False,
                 mask_seed=None, lr=1e-4, save_path=None, device=False):
        self.model = model
        ...
        self.optical_encoder = CASSI(input_shape, stride, patch_size, mask_path, trainable_mask, mask_seed,
                                     is_patch=True, y_norm=y_norm)
        ...
        if model == 'unetcompiled':
            ...
            self.criterion = nn.MSELoss()
            self.optimizer = torch.optim.Adam(self.computational_decoder.parameters(), lr=lr)
            self.scheduler = None  # torch.optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=0.99)
            # self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=0.85,
            #                                                             patience=40)
