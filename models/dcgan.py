...
class DCGAN:
    def __init__(self, batch_size, feature=64, num_ch=31, nz=64, ngf=128, ndf=128, nc=3, lr=2e-4, is_autoencoder=False,
                 save_path=None, reg_param = 1e-3, device='cuda'):
        ...
        self.generator = self.build_generator(nz=nz, ngf=ngf, nc=nc if is_autoencoder else num_ch)
        self.discriminator = self.build_discriminator(ndf=ndf, nc=nc if is_autoencoder else num_ch)
    ...
    def forward(self, dataloader, epoch, max_epochs, kind, colour):
        ...
        for _, data in data_loop:
            ...
            dict_metrics = dict(errD=errD_losses.avg, errG=errG_losses.avg, D_x=D_x_losses.avg,
                                D_G_z1=D_G_z1_losses.avg, D_G_z2=D_G_z2_losses.avg,reg_var = reg_var_losses.avg)
    ...
    def save_images(self, val_loader, epoch, save=True, show=False):
        ...
        if self.is_autoencoder:
            ...
            for i in indices:
                ...
                plt.imshow(np.clip(generated_embedded[i], 0, 1) ** 0.5)
                plt.axis('off')
    ...
    def load_checkpoint(self, path, epoch=None, is_autoencoder=False, is_gan=False,reg_param_gan=1e-3,reg_param_ae=1e-3):
        if epoch is None:
            if is_autoencoder:
