...
class LCGAN:
    def __init__(self, batch_size, latent_dim, num_classes, epochs, ema_losses, toggle_grads, in_channels, out_channels,
        ...
        self.cs = ColourSystem(cs='sRGB', start=400, end=720, num=31)
...
class Generator(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes, latent_dim, lr=0.00005, beta1=0.5, beta2=0.999,
                 adam_eps=1e-8):
        ...
        self.output = nn.Sequential(BN(output_size=out_channels[-1]), nn.ReLU(), self.which_conv(out_channels[-1], 7))
